...
Hermes loads these skills through plugin registration, not by adding the shared `skills/` directory to `skills.external_dirs`. The adapter derives process-local temporary SKILL files from the shared Git source, adds manual/dependency and permission guidance, and replaces other-host help with native Hermes help. Temporary output is not a maintained skill library; registration rebuilds it, and process exit removes it. Restart or reload the plugin after changing its source. The mode hook uses the same rendering function. Shared upstream bodies and other-platform generated files stay unchanged; an unrecognized upstream help layout fails closed for review instead of serving wrong-host update instructions.
