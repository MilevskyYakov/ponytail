...
Hermes loads these skills through plugin registration, not by adding the shared `skills/` directory to `skills.external_dirs`. The adapter derives process-local temporary SKILL files from the shared Git source, adds short descriptions, manual/dependency metadata and permission guidance, and replaces other-host help with native Hermes help. Temporary output is not a maintained skill library; registration rebuilds it, and process exit removes it. Restart or reload the plugin after changing its source. The mode hook uses the same rendering function. Shared upstream SKILL files, including their descriptions, and other-platform generated files stay unchanged; unrecognized metadata, help or debt layouts fail closed for review instead of serving unverified instructions.

This local routing/adaptation work belongs to [GERDA2.0 #371](https://github.com/MilevskyYakov/GERDA2.0/issues/371). Skill/help reads do not authorize installs, updates, delegation, file writes or publication. Live plugin updates remain separately approved operations.
